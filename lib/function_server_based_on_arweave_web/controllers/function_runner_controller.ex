defmodule FunctionServerBasedOnArweaveWeb.FunctionRunnerController do
  ...
  def run(conn, payload) do
    ...
    payload =
      with true <- check_white_list(tx_id),
        {:ok, %{code: code, if_record: if_record}} <- CodeRunner.get_ex_by_tx_id(ArweaveNode.get_node(), tx_id)
        do
          code
  ...
  def do_run(code, params, tx_id, if_record) do
    res = %{output: output} =
      ...
      |> Map.put(:code_online, ArweaveNode.get_node() <> "/" <> tx_id)
  ...
  def check_white_list(tx_id) do
    {:ok, %{raw_data: %{"owner" => owner}}} =
      ArweaveSdkEx.get_tx(ArweaveNode.get_node(), tx_id)
    owner in @white_list
  end
